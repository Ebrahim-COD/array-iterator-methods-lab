...
sortedByBirthYear = inventors.sort((first,second) => {
    return first.year - second.year  //comparing the first year to the 2nd year.
})
...
inventorNamedAda = inventors.find(finds => {
    return finds.first === 'Ada'
})
...
firstLast = people.map(change => {
    let changing = change.split(' ')
    return changing[1] + " " + changing[0]
})
...
commentById = comments.find(finds => {
    return finds.id === 823423
})
